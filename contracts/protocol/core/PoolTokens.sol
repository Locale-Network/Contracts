...
contract PoolTokens is IPoolTokens, ERC721PresetMinterPauserAutoIdUpgradeSafe {
  ...
  */
  ...
  function __initialize__(address owner, LocaleLendingConfig _config) external initializer {
    ...
    __ERC721_init_unchained("LocaleLending V2 Pool Tokens", "LLP-V2-PT");
  ...
  /**
   * @notice Called by the LocaleLendingFactory to register the pool as a valid pool. Only valid pools can mint/redeem
   * tokens
   * @param newPool The address of the newly created pool
   */
  function onPoolCreated(address newPool) external override onlyLocaleLendingFactory {
  ...
  modifier onlyLocaleLendingFactory() {
    require(_msgSender() == config.localeLendingFactoryAddress(), "Only LocaleLending factory is allowed");
    _;
  }
