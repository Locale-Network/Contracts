...
contract Lldu is ERC20PresetMinterPauserUpgradeSafe {
  bytes32 public constant OWNER_ROLE = keccak256("OWNER_ROLE");
  // $1 threshold to handle potential rounding errors, from differing decimals on Lldu and USDC;
  ...
  */
  ...
  function canMint(uint256 newAmount) internal view returns (bool) {
    ...
    uint256 liabilities = totalSupply().add(newAmount).mul(seniorPool.sharePrice()).div(llduMantissa());
    uint256 liabilitiesInDollars = llduToUSDC(liabilities);
  ...
  function canBurn(uint256 amountToBurn) internal view returns (bool) {
    ISeniorPool seniorPool = config.getSeniorPool();
    uint256 liabilities = totalSupply().sub(amountToBurn).mul(seniorPool.sharePrice()).div(llduMantissa());
    uint256 liabilitiesInDollars = llduToUSDC(liabilities);
    uint256 _assets = seniorPool.assets();
    if (_assets >= liabilitiesInDollars) {
      return true;
  ...
  function updateLocaleLendingConfig() external {
