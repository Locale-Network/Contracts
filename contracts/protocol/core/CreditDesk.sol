...
contract CreditDesk is BaseUpgradeablePausable, ICreditDesk {
  ...
  function migrateV1CreditLine(
    ...
    address pool = getLocaleLendingFactory().createMigratedPool(
      borrower,
  ...
  function getLocaleLendingFactory() internal view returns (LocaleLendingFactory) {
    return LocaleLendingFactory(config.getAddress(uint256(ConfigOptions.Addresses.LocaleLendingFactory)));
  }
