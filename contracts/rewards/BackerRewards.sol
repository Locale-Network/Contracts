...
// equation ref `_calculateNewGrossLLPRewardsForInterestAmount()`:
// (sqrtNewTotalInterest - sqrtOrigTotalInterest) / sqrtMaxInterestDollarsEligible * (totalRewards / totalLLPSupply)
...
contract BackerRewards is IBackerRewards, BaseUpgradeablePausable, SafeERC20Transfer {
  ...
  uint256 public totalRewards; // total amount of LLP rewards available, times 1e18
  ...
  uint256 public totalRewardPercentOfTotalLLP; // totalRewards/totalLLPSupply, times 1e18
  ...
  function __initialize__(address owner, LocaleLendingConfig _config) public initializer {
  ...
  /**
   * @notice Set the total gfi rewards and the % of total LLP
   * @param _totalRewards The amount of LLP rewards available, expects 10^18 value
   */
  function setTotalRewards(uint256 _totalRewards) public onlyAdmin {
    totalRewards = _totalRewards;
    uint256 totalLLPSupply = config.getLLP().totalSupply();
    totalRewardPercentOfTotalLLP = _totalRewards.mul(mantissa()).div(totalLLPSupply).mul(100);
    emit BackerRewardsSetTotalRewards(_msgSender(), _totalRewards, totalRewardPercentOfTotalLLP);
  }
  ...
  /**
   * @notice Set the max dollars across the entire protocol that are eligible for LLP rewards
   * @param _maxInterestDollarsEligible The amount of interest dollars eligible for LLP rewards, expects 10^18 value
   */
   ...
   * @param tokenId Pool token id
   * @return The amount of LLP claimable
   */
  ...
  function withdraw(uint256 tokenId) public {
    ...
    safeERC20Transfer(config.getLLP(), poolTokens.ownerOf(tokenId), totalClaimableRewards);
  ...
  function _allocateRewards(uint256 _interestPaymentAmount) internal {
    ...
    // Gross LLP Rewards earned for incoming interest dollars
    uint256 newGrossRewards = _calculateNewGrossLLPRewardsForInterestAmount(_interestPaymentAmount);
   ...
   */
  function _calculateNewGrossLLPRewardsForInterestAmount(uint256 _interestPaymentAmount)
    internal
    view
    returns (uint256)
  {
    ...
    /*
      ...
      example math:
        ...
        * 100000000000000000000000000
        / 10^18
        = 6708203932437400000000 (6,708.2039 LLP)
    */
    ...
    uint256 newGrossRewards = sqrtDiff
      .mul(totalRewardPercentOfTotalLLP)
  ...
  event BackerRewardsSetTotalRewards(address indexed owner, uint256 totalRewards, uint256 totalRewardPercentOfTotalLLP);
