...
import "../protocol/core/LocaleLendingConfig.sol";
...
contract StakingRewards is ERC721PresetMinterPauserAutoIdUpgradeSafe, ReentrancyGuardUpgradeSafe {
  ...
  function __initialize__(address owner, LocaleLendingConfig _config) external initializer {
    ...
    __ERC721_init_unchained("LocaleLending V2 LP Staking Tokens", "LLP-V2-LPS");
  ...
  function depositAndStake(uint256 usdcAmount) public nonReentrant whenNotPaused updateReward(0) {
    uint256 llduAmount = depositToSeniorPool(usdcAmount);
    uint256 lockedUntil = 0;
    uint256 tokenId = _stakeWithLockup(address(this), msg.sender, llduAmount, lockedUntil, MULTIPLIER_DECIMALS);
    emit DepositedAndStaked(msg.sender, usdcAmount, tokenId, llduAmount, lockedUntil, MULTIPLIER_DECIMALS);
  }

  function depositToSeniorPool(uint256 usdcAmount) internal returns (uint256 llduAmount) {
    ...
    updateReward(0)
  {
    ...
    uint256 leverageMultiplier = getLeverageMultiplier(lockupPeriod);
    uint256 lockedUntil = block.timestamp.add(lockDuration);
    uint256 tokenId = _stakeWithLockup(address(this), msg.sender, llduAmount, lockedUntil, leverageMultiplier);
    emit DepositedAndStaked(msg.sender, usdcAmount, tokenId, llduAmount, lockedUntil, leverageMultiplier);
  }
  ...
  function unstakeAndWithdraw(uint256 tokenId, uint256 usdcAmount) public nonReentrant whenNotPaused {
    (uint256 usdcReceivedAmount, uint256 llduAmount) = _unstakeAndWithdraw(tokenId, usdcAmount);

    emit UnstakedAndWithdrew(msg.sender, usdcReceivedAmount, tokenId, llduAmount);
  }

  function _unstakeAndWithdraw(uint256 tokenId, uint256 usdcAmount)
    internal
    updateReward(tokenId)
    returns (uint256 usdcAmountReceived, uint256 llduUsed)
  {
    require(config.getGo().goSeniorPool(msg.sender), "This address has not been go-listed");
    ISeniorPool seniorPool = config.getSeniorPool();
    ...
    llduUsed = llduBalanceBefore.sub(lldu.balanceOf(address(this)));

    _unstake(tokenId, llduUsed);
    config.getUSDC().safeTransfer(msg.sender, usdcAmountReceived);

    return (usdcAmountReceived, llduUsed);
  }
    ...
    whenNotPaused
  {
    ...
    uint256[] memory llduAmounts = new uint256[](usdcAmounts.length);
    for (uint256 i = 0; i < usdcAmounts.length; i++) {
      (uint256 usdcReceivedAmount, uint256 llduAmount) = _unstakeAndWithdraw(tokenIds[i], usdcAmounts[i]);

      usdcReceivedAmountTotal = usdcReceivedAmountTotal.add(usdcReceivedAmount);
      llduAmounts[i] = llduAmount;
    }

    emit UnstakedAndWithdrewMultiple(msg.sender, usdcReceivedAmountTotal, tokenIds, llduAmounts);
  }

  function unstakeAndWithdrawInLldu(uint256 tokenId, uint256 llduAmount) public nonReentrant whenNotPaused {
    uint256 usdcReceivedAmount = _unstakeAndWithdrawInLldu(tokenId, llduAmount);

    emit UnstakedAndWithdrew(msg.sender, usdcReceivedAmount, tokenId, llduAmount);
  }

  function _unstakeAndWithdrawInLldu(uint256 tokenId, uint256 llduAmount)
    internal
    updateReward(tokenId)
    returns (uint256 usdcReceivedAmount)
  {
    usdcReceivedAmount = config.getSeniorPool().withdrawInLldu(llduAmount);
    _unstake(tokenId, llduAmount);
    config.getUSDC().safeTransfer(msg.sender, usdcReceivedAmount);
    return usdcReceivedAmount;
  }

  function unstakeAndWithdrawMultipleInLldu(uint256[] calldata tokenIds, uint256[] calldata llduAmounts)
    public
    nonReentrant
    whenNotPaused
  {
    require(tokenIds.length == llduAmounts.length, "tokenIds and usdcAmounts must be the same length");

    uint256 usdcReceivedAmountTotal = 0;
    for (uint256 i = 0; i < llduAmounts.length; i++) {
      uint256 usdcReceivedAmount = _unstakeAndWithdrawInLldu(tokenIds[i], llduAmounts[i]);
  ...
  function exitAndWithdraw(uint256 tokenId) external {
    unstakeAndWithdrawInLldu(tokenId, positions[tokenId].amount);
    getReward(tokenId);
  }
  ...
  function updateLocaleLendingConfig() external onlyAdmin {
    ...
    emit LocaleLendingConfigUpdated(_msgSender(), address(config));
  }
